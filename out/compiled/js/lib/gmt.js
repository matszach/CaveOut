...
const Gmt = {
    ...
    Perlin: {
        ...
        v2d(x, y) {
            ...
            return Gmt.Perlin._lerp(Gmt.Perlin._lerp(n00, n10, u), Gmt.Perlin._lerp(n01, n11, u), Gmt.Perlin._fade(y));
        },
        map2d(options) {
    ...
    Segment: class {
        ...
        copy() {
            return new Gmt.Segment(this.start.x, this.start.y, this.end.x, this.end.y);
        }
        length() {
    ...
    Intersection: {
        ...
        segments(s1, s2) {
            ...
            return {
                ...
                intersect: Gmt.between(t, 0, 1) && Gmt.between(u, 0, 1),
                t: t,
                u: u
            };
        },
    ...
    TileSet: class {
        ...
        get(x, y) {
            return new Gmt.Tile(this.img, x * (this.xSize + this.borderSize), y * (this.ySize + this.borderSize), this.xSize, this.ySize);
        }
    },
    ImageWrapper: class {
        constructor(fileUrl) {
    ...
    CanvasWrapper: class {
         ...
         */
        getBoundingRect() {
            return new Gmt.Rectangle(-this.offsetX / this.unit, -this.offsetY / this.unit, this.canvas.width / this.unit, this.canvas.height / this.unit);
        }
        /**
        ...
        strokeSegmentsOpen(segments, color, lineWidth) {
            ...
            this.context.beginPath();
            let sts = segments[0];
            this.context.moveTo(sts.start.x * this.unit + this.offsetX, sts.start.y * this.unit + this.offsetY);
        ...
        fillPolygon(polygon, color) {
            ...
            let vs = vertices[0];
            this.context.beginPath();
            this.context.moveTo(vs.x * this.unit + this.offsetX, vs.y * this.unit + this.offsetY);
            for (let i = 1; i < vertices.length; i++) {
                let v = vertices[i];
                this.context.lineTo(v.x * this.unit + this.offsetX, v.y * this.unit + this.offsetY);
            }
            this.context.closePath();
            this.context.fill();
            return this;
        ...
        fillCircle(circle, color) {
            this.setFillStyle(color);
            this.context.beginPath();
            this.context.arc(circle.x * this.unit + this.offsetX, circle.y * this.unit + this.offsetY, circle.radius * this.unit, 0, Gmt.PI * 2);
            this.context.fill();
            return this;
        }
        ...
        write(content, x, y, color, size, font, rotation, alpha) {
            ...
            this.context.translate(x * this.unit + this.offsetX, y * this.unit + this.offsetY);
        ...
        drawTile(tile, targetRect, rotation, alpha) {
            ...
            this.context.save();
            this.context.globalAlpha = alpha || 1;
            this.context.translate(targetRect.x * this.unit + this.offsetX, targetRect.y * this.unit + this.offsetY);
            this.context.rotate(rotation || 0);
            // drawing proper
            this.context.drawImage(tile.img, tile.boudingRect.x, tile.boudingRect.y, tile.boudingRect.width, tile.boudingRect.height, -targetRect.width / 2 * this.unit, -targetRect.height / 2 * this.unit, targetRect.width * this.unit, targetRect.height * this.unit);
            // context setting restore
            this.context.restore();
            return this;
        }
        ...
        linearGradient(startVertex, endVertex, ...colorInfo) {
            let grd = this.context.createLinearGradient(startVertex.x * this.unit + this.offsetX, startVertex.y * this.unit + this.offsetY, endVertex.x * this.unit + this.offsetX, endVertex.y * this.unit + this.offsetY);
        ...
        radialGradient(startCircle, endCircle, ...colorInfo) {
            let grd = this.context.createRadialGradient(startCircle.x * this.unit + this.offsetX, startCircle.y * this.unit + this.offsetY, startCircle.radius * this.unit, endCircle.x * this.unit + this.offsetX, endCircle.y * this.unit + this.offsetY, endCircle.radius * this.unit);
            for (let i = 0; i < colorInfo.length;) {
                grd.addColorStop(colorInfo[i++], colorInfo[i++]);
            }
            return grd;
    ...
    Input: {
        ...
        mousePosVertexCW(cw) {
            return new Gmt.Vertex((this._mouse.posX - cw.offsetX) / cw.unit, (this._mouse.posY - cw.offsetY) / cw.unit);
        },
