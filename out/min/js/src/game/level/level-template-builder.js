...
return false;}
putRect(initX,initY,width,height,value){for(let x=initX;x<initX+width;x++){for(let y=initY;y<initY+height;y++){this.safePut(x,y,value);}}}
generateCaveLevel(yLevel){const y=new Variator(yLevel-6,yLevel+6);const offsetTop=new Variator(3,10);const offsetBottom=new Variator(3,10);const levelCore=[];for(let x=0;x<this.template.xSize;x++){levelCore.push(y.get());const initX=x;const initY=y.get()-offsetTop.get();const width=1;const height=offsetBottom.get()+offsetTop.get();this.putRect(initX,initY,width,height,BLOCK_TYPES.NONE);y.change(0.5);offsetTop.change(0.35);offsetBottom.change(0.2);}
return levelCore;}
